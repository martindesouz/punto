...
const handler: Handler = async (req, res) => {
  ...
  if (!isAllowedGuess(guess)) return sendJson(res, 422, { error: 'not_a_word' })
  ...
  if (won || lost) {
    ...
    state.won = won
    state.end = Date.now()
    const result = scoreGame(won, state.guesses.length, state.hints.length, state.end - state.start)
